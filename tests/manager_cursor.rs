//! Integration tests for `mcp cursor {enable, disable, list}` plus the
//! Cursor-specific JSON shape (`CursorConfig` / `CursorServer`) and the
//! `--workspace` flag that switches between user mode (`$HOME/.cursor/mcp.json`)
//! and workspace mode (`$CWD/.cursor/mcp.json`).
//!
